...
LIFE-OS SEARCH POLICY (REQUIRED)
<life_os_search>
<sources>
<source>CLI Claude Flow Memory (global) -> npx claude-flow@v3alpha memory search -q "{query}"</source>
<source>Local MD search -> rg "{query}" {bmb_creations_output_folder}/life-os/</source>
<source>Web/MCP search for best practices</source>
</sources>
<order>1) CLI memory 2) Local MD 3) Web/MCP</order>
<note>MCP memory tools are LOCAL only (Issue #967). Use CLI for global memory.</note>
<trigger>Run when a decision, ambiguity, or choice is required.</trigger>
</life_os_search>

CONSILIUM DECISION PROTOCOL
<consilium_protocol>
<after_search>Convene 2-4 expert perspectives to rank options</after_search>
<output>Present best 2-4 options with pros/cons + recommendation + confidence (high/medium/low)</output>
<evidence>Record a short evidence snapshot before deciding</evidence>
<user_choice>Ask user to choose or override</user_choice>
</consilium_protocol>

CONFIDENCE RULE (HEURISTIC)
<confidence_rule>
<high>2+ strong sources, consistent findings</high>
<medium>1 strong + 1 weak source, partial alignment</medium>
<low>conflicting sources or weak evidence only</low>
</confidence_rule>

CONFIDENCE ESTIMATION (AUTOMATIC)
<confidence_estimation>
<inputs>
<input>evidence_count</input>
<input>source_quality</input>
<input>consistency</input>
</inputs>
<logic>
<pseudocode>
def estimate_confidence(evidence_count, source_quality, consistency):
    if evidence_count >= 2 and source_quality == 'strong' and consistency == 'high':
        return 'high'
    if evidence_count >= 1 and consistency in ['medium','high']:
        return 'medium'
    return 'low'
</pseudocode>
</logic>
